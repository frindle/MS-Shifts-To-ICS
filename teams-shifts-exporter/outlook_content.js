// outlook_content.js — injected into outlook.office.com
...
(function () {
  ...
  async function autoFocusWorkShifts() {
    const { focusWorkShifts } = await new Promise(r => chrome.storage.local.get('focusWorkShifts', r));
    if (!focusWorkShifts) return;
    chrome.storage.local.remove('focusWorkShifts');

    // Wait for the calendar sidebar to load
    for (let attempt = 0; attempt < 20; attempt++) {
      await sleep(500);
      // Find all calendar checkboxes in the left sidebar
  ...
  function findMSALTokens() {
    ...
    //    Key format: msal.2.token.keys.{clientId}
    //    Value: {"idToken":[...],"accessToken":[...],"refreshToken":[...]}
    ...
    // ④ Fallback: scan ALL keys case-insensitively for 'accesstoken'
    //    MSAL v2 uses lowercase 'accesstoken' in cache keys, so
    //    the previous PascalCase/camelCase checks silently missed every token.
  ...
  // ─── ICS Parser ───────────────────────────────────────────────────────────
  // Extracts VEVENT blocks from an ICS string into plain objects.
  ...
  // Converts ICS datetime (20260806T124500Z) → ISO 8601 (2026-08-06T12:45:00Z)
  ...
  // ─── Graph API Import ─────────────────────────────────────────────────────
  // Uses the MSAL token already in OWA's session to create events directly.
  ...
  async function importViaGraphAPI(icsContent, onProgress) {
    ...
    for (let i = 0; i < newEvents.length; i += 20) {
      ...
      // Outlook REST API doesn't support $batch — POST events one at a time
      if (isPascal) {
  ...
  // ─── Fallback UI Helpers ──────────────────────────────────────────────────
  // Used only when the Graph API token is unavailable.
  ...
  async function importICS(icsContent, overlay) {
    ...
    // Fallback: UI automation (requires the add-calendar dialog flow)
