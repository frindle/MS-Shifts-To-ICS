...
(function () {
  ...
  async function scrape(options = {}) {
    ...
    try {
      // Only navigate from the top frame; inside the iframe we call the API directly
      if (window === window.top) await navigateToShifts();

      // Determine API base URL from current location
      // e.g. https://flw.teams.cloud.microsoft/svc-nam1/...
      const region = window.location.pathname.split('/').filter(Boolean)[0];
      ...
      // Fetch all shifts from today → target end date
      ...
      // Parse events — try multiple field name shapes defensively
      const events = [];
